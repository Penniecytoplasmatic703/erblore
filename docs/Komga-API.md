...
Erblore provides a Komga-compatible API that allows you to use Komga clients (like Tachiyomi, Tachidesk, Komelia, etc.) to access your Erblore library.

## Features

The Komga API implementation in Erblore provides the following endpoints:
...
Erblore organizes books differently than Komga:

- **Komga**: Libraries → Series → Books
- **Erblore**: Libraries → Books (with optional series metadata)
...
1. Navigate to **Settings** in Erblore
...
1. Create an OPDS user account in Erblore settings
...
3. Configure the source:
   - Server URL: `http://your-erblore-server/`
...
- Read progress tracking from Komga clients is not synchronized with Erblore
...
- Ensure the Komga API is enabled in Erblore settings
...
- The Komga API uses OPDS user accounts, not your main Erblore account
- Create an OPDS user in the Settings → OPDS section
- Use those credentials in your Komga client

## API Compatibility

The Erblore Komga API aims to be compatible with Komga v1.x API. While not all endpoints are implemented, the core functionality needed for reading and browsing is supported.
